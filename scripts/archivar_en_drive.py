...
archiva solo** en la carpeta que le corresponde según el destino.
    ...
    factura_estado.destino  ->  maestro_destinos.drive_carpeta
                            ->  COMPRAS / 2026 / 8. Agosto / BOG001 /
                                Amande_A12623_29072026_BOG001.pdf

La ruta NO se inventa: `drive_carpeta` se sembró de dónde el equipo VENÍA
guardando cada destino (franquicias bajo `FRANQUICIADOS/`, propias y
transversales en la raíz del mes). El nombre sigue la convención que ya usaban.
Las carpetas que falten se crean; una carpeta vacía solo significa que nadie
clasificó nada ahí todavía.

Qué NO hace, a propósito:
  · No toca lo que compras ya archivó a mano (`origen='compras'`): si el
    documento ya está en esa carpeta, lo deja quieto.
  · No archiva sin destino. Sin clasificación no hay ruta, y adivinarla sería
    meter plata en la carpeta equivocada.
  · No inventa carpeta para un destino que no la tiene mapeada: lo reporta.
...
SELECT f.cufe, f.nombre_proveedor, f.numero, f.fecha_emision, f.link_drive,
       e.destino, d.drive_carpeta,
       -- El código para el nombre del archivo es SIEMPRE la última hoja de la
       -- ruta ('FRANQUICIADOS/PER001' -> PER001), no el short_code: es lo que el
       -- equipo escribe al final del nombre y lo que el ingest vuelve a leer.
       upper(split_part(d.drive_carpeta, '/', -1))            AS destino_cod
  FROM facturas f
  JOIN factura_estado e USING (cufe)
  LEFT JOIN maestro_destinos d
         ON d.activo AND upper(d.nombre) = upper(e.destino)
 WHERE f.fecha_emision >= %(ini)s AND f.fecha_emision < %(fin)s
   AND e.destino IS NOT NULL AND e.destino <> ''
   -- ya archivado (por nosotros o por compras) en ESA carpeta: no se repite
   AND NOT EXISTS (
         SELECT 1 FROM factura_soportes s
          WHERE s.cufe = f.cufe
            AND (d.drive_carpeta IS NULL
                 OR upper(s.destino_carpeta) = upper(split_part(d.drive_carpeta, '/', -1))))
...
def main() -> int:
    ...
    filas = cur.fetchall()
    print(f"→ {anio}-{mes:02d}: {len(filas)} facturas clasificadas sin archivar")
    if not filas:
        print("Nada que archivar.")
        return 0

    token = drive_token()
    mes_id = carpeta_mes(token, anio, mes, crear=args.commit)
    if not mes_id:
        # En dry-run seguimos: el reporte de qué se archivaría vale igual, y no
        # queremos crear carpetas solo para poder mirar.
        print(f"(la carpeta del mes {anio}-{mes:02d} todavía no existe en Drive; "
              "se crea al correr con --commit)")

    cache: dict = {}
    res = Counter()
    sin_ruta: Counter = Counter()
    sin_doc: list[str] = []
    archivados: list[tuple] = []

    for (cufe, prov, numero, fecha, link, destino, ruta, destino_cod) in filas[: args.limit or None]:
        if not ruta:
        ...
        nombre = nombre_archivo(prov, numero, fecha, destino_cod or "SIN_DESTINO", ext)
        ...
        carp = carpeta_destino(token, mes_id, ruta, True, cache)
        ...
        archivados.append((copia["id"], nombre,
                           ...
                           ruta, anio, mes, (destino_cod or "").upper(),
    ...
    if archivados:
        ...
        registrar_evento(cur, cufe=None, tipo="archivo_drive",
                         valor_nuevo={"periodo": f"{anio}-{mes:02d}",
